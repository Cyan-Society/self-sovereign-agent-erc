"""
Simple version of the anchor_state tool for Letta ADE.

Copy this entire function into Letta ADE's custom tool creator.
Make sure the Letta server has these environment variables set:
- AGENT_CONTRACT_ADDRESS
- AGENT_EXECUTOR_PRIVATE_KEY
- LETTA_PASSWORD (for self-hosted)
"""

def anchor_my_state(token_id: int = 1) -> str:
    """
    ...
    LETTA_PASSWORD = os.getenv("LETTA_PASSWORD", "TWIJftq/ufbbxo8w51m/BQ1wBNrZb/JTlmnopxyz")
    ...
    try:
        ...
        return f"""State anchored!
Hash: 0x{state_hash.hex()[:32]}...
TX: {tx_hash.hex()}
Gas: {receipt['gasUsed']} (~${float(cost_eth)*4000:.4f})
Memory: {list(memory_blocks.keys())}
Archival: {len(archival_entries)} entries"""
